...
interface EventRow {
  ...
  end_at: string | null;
  created_at: string;
  form_schema_json: Array<{ type?: string }>;
...
const isEventCompleted = (event: EventRow, now: Date) => {
...
const eventHasProFields = (formSchema: Array<{ type?: string }>) =>
...
async function fetchEventAnalytics(from?: string, to?: string): Promise<EventAnalyticsData> {
  ...
  const eventsList = (events as EventRow[] | null) || [];
  const registrationsList = (registrations as RegistrationRow[] | null) || [];
